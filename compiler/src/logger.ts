...
export function createLogger(initializer: string | number, initialDebug: boolean | (() => boolean) = false, ...transforms: LogItemTransformer[]) {
  let prefix = typeof initializer === "string" ? initializer : "  ".repeat(initializer);
  const transform: LogItemTransformer | undefined = transforms.length > 0 ? transforms.reduce((prev, curr) => (item, idx) => curr(prev(item, idx), idx), (item: any) => item) : undefined;
  return createLoggerInternal(prefix, initialDebug, transform);

  function createLoggerInternal(prefix: string, debugOrFunc: boolean | (() => boolean), transform?: LogItemTransformer): Logger {
    ...
    const log: Logger = (...args: any[]) => {
      ...
      console.log(header, ...mapped.map((item, idx) => {
        // const first = `{${idx}}`;
    ...
    log.withTransform = (...transforms: LogItemTransformer[]) => {
      const composed = transforms.length > 0 ? transforms.reduce((prev, curr) => (item, idx) => curr(prev(item, idx), idx), (item: any) => item) : undefined;
