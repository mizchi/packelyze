...
const createBundleTransformerFactory: (checker: TS.TypeChecker) => TS.TransformerFactory<any> = (checker) => (context) => {
    ...
    if (
      ...
      for (const specifier of node.importClause.namedBindings.elements) {
        ...
        if (outerSymbol?.valueDeclaration && (
          ...
          const clonedFunction = cloneImportedFunction(outerSymbol.valueDeclaration, specifier.name.text, specifier.propertyName?.text);
        ...
        if (outerSymbol?.valueDeclaration && (
          ...
          const clonedFunction = cloneImportedClass(outerSymbol.valueDeclaration, specifier.name.text, specifier.propertyName?.text);
          if (clonedFunction) {
            hoistedDeclarations.push(clonedFunction);
          }
        }
  ...
  function cloneImportedFunction(node: TS.FunctionDeclaration | TS.FunctionExpression, originalName: string, nameAs?: string): Array<TS.VariableStatement | TS.FunctionDeclaration> {
    if (node.body == null) {
      return [];
    }
  ...
  function cloneImportedClass( node: TS.ClassDeclaration | TS.ClassExpression, originalName: string, nameAs?: string): TS.VariableStatement | TS.ClassDeclaration | undefined {
    if (nameAs == null) {
