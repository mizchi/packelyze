...
import { createIncrementalLanguageServiceHost, createIncrementalLanguageService, IncrementalSnapshot } from "./services";
...
test("IncrementalLanguageService", () => {
  ...
  expect(
    service.getSemanticDiagnostics(indexFilePath).length,
  ).toBe(1);
  ...
  // console.log("---------------- changes");
  expect(
    host.getScriptVersion(indexFilePath)
  ).toBe("1");
  // service.logger.on();
  // host.logger.on();
  service.writeSnapshotContent(
  ...
  expect(
    service.readSnapshotContent(indexFilePath)
  ).toBe("const x: number = 2;");
  ...
  expect(
    s3.getText(0, s3.getLength())
  ).toBe("const x: number = 4;");
  ...
  expect(
    service.getSemanticDiagnostics(newFilePath).length,
  ).toBe(1);
  ...
  // create file under virtual directory
  {
    ...
    expect(
      host.readDirectory!(dirname)
    ).toEqual(["new.ts", "new2.ts"]);
