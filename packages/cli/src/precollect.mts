...
const domReservedFiles = ["lib.dom.d.ts", "lib.dom.iterable.d.ts"];

const workerReservedFiles = ["lib.webworker.d.ts", "lib.webworker.importscripts.d.ts"];
...
export async function getDomReserved(tsLibDir: string, cssReserved: Set<string>) {
  const filepaths = await fs.readdir(tsLibDir);
  const dtsPaths = filepaths
    .filter((t) => t.startsWith("lib.") && t.endsWith(".d.ts"))
...
export async function getCloudflareWorkersReserved(cloudflareWorkersTypesDtsPath: string) {
...
export async function getNodeReserved(nodeDtsDir: string) {
  const filesInNode = await fs.readdir(nodeDtsDir, {
    withFileTypes: true,
  });
  const nodeDtsPathList = filesInNode
    .filter((x) => x.isFile() && x.name.endsWith(".d.ts"))
  ...
  for (const nodeDtsPath of nodeDtsPathList) {
    ...
    const source = ts.createSourceFile(nodeDtsPath, code, ts.ScriptTarget.ES2019, true);
    const result = collectProperties(source);
...
export async function getCssReserved(cssDtsPath: string) {
...
export async function getReactReserved(reactDtsDir: string, cssReserved: Set<string>) {
  const filesInReact = await fs.readdir(reactDtsDir, {
    withFileTypes: true,
  });
  const reactDtsPathList = filesInReact
    .filter((x) => x.isFile() && x.name.endsWith(".d.ts"))
  ...
  for (const reactDtsPath of reactDtsPathList) {
    ...
    const source = ts.createSourceFile(reactDtsPath, code, ts.ScriptTarget.ES2019, true);
    const result = collectProperties(source);
    reserved.push(...result.reserved);
  }
...
export async function getDenoReserved(denoDtsDir: string) {
