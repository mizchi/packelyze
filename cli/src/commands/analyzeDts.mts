...
export async function analyzeDts() {
  ...
  try {
    ...
    if (args.values.config) {
      ...
      if (fs.existsSync(configPath)) {
        try {
          ...
          console.info(
            "[packelyze] skip by parse error - packelyze.config.json",
            e,
          );
        }
      } else {
    ...
    if (writeDts) {
      ...
      console.log(
        "[packelyze:analyze-dts:generate]",
        dtsPath.replace(cwd + "/", ""),
      );
    }
    ...
    if (argsValues.builtins) {
      ...
      for (const builtinName of argsValues.builtins) {
        if (builtinName in builtins) {
          console.log(
            "[packelyze:analyze-dts:include-builtins]",
            builtinName,
    ...
    const localAmbientModules = getLocalAmbientModules(
      program,
      cwd,
    );
    if (debug) {
      console.log(
        ...
        program.getSourceFiles()
          .filter((m) => {
            return m.fileName.endsWith(".d.ts") &&
